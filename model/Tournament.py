...
class Tournament():
    ...
    def validate_round(self, round_number: int):
        for match in self.rounds[round_number]:
            if match.winner == 'TBD':
                print(
                    f"Error: Cannot validate the round, cause the match {match.print_match()} has the unexpected winner {match.winner}")
                exit(-1)
        self.actual_round = self.actual_round + 1
        self.matches = []
        for match_number in range(int(len(self.rounds[self.actual_round-1])/2)):
            new_match = Match(self.rounds[self.actual_round-1][match_number*2].winner,
                              self.rounds[self.actual_round-1][(match_number*2)+1].winner)
    ...
    def random_result_round(self):
        for match in self.matches:
            if type(match.team1) == Team and type(match.team2) == Team:
                ...
                print(
                    f"Error : Unexpected error, the match {match} seems to not have valid teams : {match.team1} - {match.team2} ")
